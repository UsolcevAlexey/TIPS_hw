...
def p(state, params):
    """
    расчет управляющего воздействия на основе П-регуляторования
    :param state: состояния ОУ
    :param params: параметры П-регуляторов
    :return: управляющее воздействие
    """

    # Коэффициенты ПД-регулятора (изменено)
    ...
    # Формируем управляющее воздействие ПД-регулятора (изменено) 
...
if __name__ == "__main__":
    ...
    params_pd = np.array([-0.01283713, -3.11526867])
    ...
    if optimize_params:
        ...
        vid = VideoRecorder(env, path=f"random_luna_lander_p.mp4")
        #params_pd = np.array([-0.01283713, -3.11526867])
