...
def pi(state, params):
    ...
    ki_alt = params[1]  # интегральная состовляющая по x
    kp_ang = params[2]  # пропорциональная состовляющая по углу
    ki_ang = params[3]  # интегральная состовляющая по углу
    ...
    # интегральная составляющая(добавлено)
    int_alt = int_ang = 0
    int_ang += ang_error
    int_alt += alt_error

    # Формируем управляющее воздействие ПД-регулятора
    alt_adj = kp_alt * alt_error + ki_alt * int_alt # изменено
    ang_adj = kp_ang * ang_error + ki_ang * int_ang # изменено
